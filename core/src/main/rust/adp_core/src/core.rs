// use std::io;
// // use wasmer::{imports, Function, NativeFunc, ChainableNamedResolver, NamedResolver};
...
//     store: Store<WasiCtx>,
//     // size -> buffer ptr
//     pub allocate_buffer_func: TypedFunc<u32, u32>,
//     // (buffer ptr, size) -> void
//     pub deallocate_buffer_func: TypedFunc<(u32, u32), ()>,
//     // void -> context ptr
//     pub prepare_transform_func: TypedFunc<u64, u32>,
//     // context ptr -> void
//     pub transform_func: TypedFunc<u32, ()>,
//     // context ptr -> void
//     pub finalize_tansform_func: TypedFunc<(u32, u32, u32), ()>,
//     // schema32 ptr -> void
//     pub release_schema32_func: TypedFunc<u32, ()>,
//     // array32 ptr -> void
//     pub release_array32_func: TypedFunc<u32, ()>,
//     // void -> schema32
//     pub new_ffi_schema: TypedFunc<(), u32>,
//     // void -> array32
//     pub new_ffi_array: TypedFunc<(), u32>,
//     pub get_first_elem_of_tuple: TypedFunc<u32, u32>,
//     pub get_second_elem_of_tuple: TypedFunc<u32, u32>,
//     pub drop_tuple: TypedFunc<u32, ()>,
//     pub transformation_ipc: TypedFunc<(u32, u32), u32>,
// }
...
//         .unwrap()
//         .build();
//         let mut store: Store<WasiCtx> = Store::new(&engine, wasi);
//         // let mut store: Store<()> = Store::default();
        
//         let module = Module::new(&store.engine(), module_bytes).unwrap();
        
//         let release_native_schema = Func::wrap(&mut store, |schema: u32| {release_exported_schema(schema)});
//         let release_native_array = Func::wrap(&mut store, |array: u32| {release_exported_array(array)});
//         // let release_native_schema = Func::wrap(&mut store, release_exported_schema.into());
//         // let release_native_array = Func::wrap(&mut store, release_exported_array.into());
//         linker.func_wrap("env", "release_func", |schema: u32| {release_exported_schema(schema)}).unwrap();
//         linker.func_wrap("env", "release_array_func", |array: u32| {release_exported_array(array)}).unwrap();
//         // let imports = [release_native_schema.into(), release_native_array.into()];

//         let instance = linker.instantiate(&mut store, &module).unwrap();
//         // linker.instance(&mut store, "env", instance).unwrap();
        

//         // let instance = Instance::new(&mut store, &module, &imports).unwrap();

//         let allocate_buffer_func = instance
//             .get_func(&mut store, "allocate_buffer")
//             .unwrap()
//             .typed::<u32, u32, _>(&store)
//             .unwrap();

//         let deallocate_buffer_func = instance
//             .get_func(&mut store, "deallocate_buffer")
//             .unwrap()
//             .typed::<(u32, u32), (), _>(&store)
//             .unwrap();

//         let prepare_transform_func = instance
//             .get_func(&mut store, "prepare_transform")
//             .unwrap()
//             .typed::<u64, u32, _>(&store)
//             .unwrap();
...
//             .get_func(&mut store, "finalize_tansform")
//             .unwrap()
//             .typed::<(u32, u32, u32), (), _>(&store)
//             .unwrap();


//         let release_schema32_func = instance
//             .get_func(&mut store, "release_schema32")
//             .unwrap()
//             .typed::<u32, (), _>(&store)
//             .unwrap();
...
//             .get_func(&mut store, "release_array32")
//             .unwrap()
//             .typed::<u32, (), _>(&store)
//             .unwrap();

//         let new_ffi_schema = instance
//             .get_func(&mut store, "new_ffi_schema")
//             .unwrap()
//             .typed::<(), u32, _>(&store)
//             .unwrap();

//         let new_ffi_array = instance
//             .get_func(&mut store, "new_ffi_array")
//             .unwrap()
//             .typed::<(), u32, _>(&store)
//             .unwrap();

//         let get_first_elem_of_tuple = instance
//             .get_func(&mut store, "get_first_of_tuple")
//             .unwrap()
//             .typed::<u32, u32, _>(&store)
//             .unwrap();
...
//             .get_func(&mut store, "get_second_of_tuple")
//             .unwrap()
//             .typed::<u32, u32, _>(&store)
//             .unwrap();
...
//             .get_func(&mut store, "read_transform_write_from_bytes")
//             .unwrap()
//             .typed::<(u32, u32), u32, _>(&store)
//             .unwrap();
...
//         // // let wasi_env = WasiEnv::new(wasi_env);
...
//         // let resolver = import_object.chain_front(import_object1);
...
//     pub fn context(&mut self, context: u64) -> &FFI_TransformContext {
...
//     pub fn allocator_base(&mut self) -> u64 {
//         let memory = self.instance.get_memory(&mut self.store, "memory").unwrap();
//         let mem_ptr = memory.data_ptr(&mut self.store);
//         mem_ptr as u64
//     }

//     pub fn allocate_buffer(&mut self, size: u32) -> u32 {
//         self.allocate_buffer_func.call(&mut self.store, size).unwrap()
//     }

//     pub fn deallocate_buffer(&mut self, buffer_ptr: u32, size: u32) {
//         self.deallocate_buffer_func.call(&mut self.store, (buffer_ptr, size)).unwrap();
//     }

//     pub fn prepare_transform(&mut self) -> u32 {
//         let base = self.allocator_base();
//         self.prepare_transform_func.call(&mut self.store, base).unwrap()
//     }

//     pub fn transform(&mut self, context: u32) -> jptr {
//         let base = self.allocator_base();
...
//         self.transform_func.call(&mut self.store, context).unwrap();
//         // println!("schema 32 = {:?}, array 32 = {:?}", schema32, array32);
...
//     pub fn finalize_tansform(&mut self, context: u32, schema_ptr: u32, array_ptr: u32) {
//         // println!("finalize transform");
//         release_exported_schema64(0);
//         release_exported_array64(0);
//         self.finalize_tansform_func
//             .call(&mut self.store, (context, schema_ptr, array_ptr))
//             .unwrap();
//     }

//     pub fn release_schema32(&mut self, schema32: u32) {
//         self.release_schema32_func.call(&mut self.store, schema32).unwrap();
//     }

//     pub fn release_array32(&mut self, array32: u32) {
//         self.release_array32_func.call(&mut self.store, array32).unwrap();
//     }

//     pub fn new_ffi_schema(&mut self) -> u32 {
//         self.new_ffi_schema.call(&mut self.store, ()).unwrap()
//     }

//     pub fn new_ffi_array(&mut self) -> u32 {
//         self.new_ffi_array.call(&mut self.store, ()).unwrap()
//     }

//     #[no_mangle]
//     pub extern "system" fn TransformationIPC(&mut self, address: u32, length: u32) -> u32 {
...
//         let transformed_tuple = self.transformation_ipc.call(&mut self.store, (address, length)).unwrap();
//         transformed_tuple
//     }

//     #[no_mangle]
//     pub extern "system" fn GetFirstElemOfTuple(&mut self, tuple_ptr: u32) -> u32 {
//         // // Get the wasm module instance and the functions we want to use
//         // let wasm_module = Into::<Pointer<WasmModule>>::into(wasm_module_ptr).borrow();
//         // let instance = &wasm_module.instance;
//         // let get_first_of_tuple = instance.exports.get_function("get_first_of_tuple").unwrap().native::<i64, i64>().unwrap();
//         self.get_first_elem_of_tuple.call(&mut self.store, tuple_ptr).unwrap()
//     }

//     #[no_mangle]
//     pub extern "system" fn GetSecondElemOfTuple(&mut self, tuple_ptr: u32) -> u32 {
//         // Get the wasm module instance and the functions we want to use
//         // let wasm_module = Into::<Pointer<WasmModule>>::into(wasm_module_ptr).borrow();
//         // let instance = &wasm_module.instance;
//         // let get_second_of_tuple = instance.exports.get_function("get_second_of_tuple").unwrap().native::<i64, i64>().unwrap();
//         self.get_second_elem_of_tuple.call(&mut self.store, tuple_ptr).unwrap()
//     }

//     #[no_mangle]
//     pub extern "system" fn DropTuple(&mut self, tuple_ptr: u32) {
//         // Get the wasm module instance and the functions we want to use
//         // let wasm_module = Into::<Pointer<WasmModule>>::into(wasm_module_ptr).borrow();
//         // let instance = &wasm_module.instance;
//         // let drop_tuple_wasm = instance.exports.get_function("drop_tuple").unwrap().native::<i64, ()>().unwrap();
//         self.drop_tuple.call(&mut self.store, tuple_ptr).unwrap();
//     }
// }
...
pub struct CoreInstance {
    ...
    pub transformation_ipc: NativeFunc<(u32, u32, u32, u32), u32>,
}
...
impl CoreInstance {
    ...
    pub fn try_new(module_bytes: &[u8]) -> Result<Self, io::Error> {
        ...
        let instance = Instance::new(&module, &resolver).unwrap();
        // let instance = Instance::new(&module, &import_object1).unwrap();
        ...
        let transformation_ipc = instance
            .exports
            .get_native_function::<(u32, u32, u32, u32), u32>("read_transform_write_from_bytes")
            .unwrap();
    ...
    pub fn convert_64_to_32(&mut self, context: u32) {
        let base = self.allocator_base();
        // Get the schema and arrow of 64 bit from the context
        let ctx = to64(base, context) as *mut FFI_TransformContext;
        let ctx = unsafe { &mut *ctx };
...
0
    ...
    pub fn convert_32_to_64(&mut self, context: u32) -> jptr {
        let base = self.allocator_base();
        // Get the schema and arrow of 64 bit from the context
        let ctx = to64(base, context) as *mut FFI_TransformContext;
        let ctx = unsafe { &mut *ctx };
    ...
    #[no_mangle]
    pub extern "system" fn TransformationIPC(&mut self, address: u32, length: u32, conf_addr: u32, conf_len: u32) -> u32 {
        // Get the wasm module instance and the functions we want to use
        // let wasm_module = Into::<Pointer<WasmModule>>::into(wasm_module_ptr).borrow();
        // let instance = &wasm_module.instance;
        // let read_transform_write_from_bytes_wasm = instance.exports.get_function("read_transform_write_from_bytes").unwrap().native::<(i64, i64), i64>().unwrap();
        ...
        // Call the function that read the bytes in the `address` parameter and getting the appropriate record batch
        // Then, it makes a transformation, writes back the transformed record batch, and returns a tuple of `(address, len)` of the transformed batch
        let transformed_tuple = self.transformation_ipc.call(address, length, conf_addr, conf_len).unwrap();
        transformed_tuple
    }
